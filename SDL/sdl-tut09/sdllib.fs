...
[vmodes] 256x256x16            \ custom for a Linux Journal 4401 demo
[vmodes] 320x200x16
[vmodes] 640x480x16
[vmodes] 800x600x16
[vmodes] 1024x768x16
...
: SDLLib-Version ( -- )
  ." +-------------------------------------------------+" cr
  ." |   SDLLib v1.0d 04/15/2010 by Timothy Trussell   |" cr
  ." +-------------------------------------------------+" cr
;
...
\ ---[ Draw-Filled-Box ]---------------------------------------------
\ Draws a filled box to the surface, in the specified color.
\ I will use the fast HLine function to fill each line quickly.
\ -------------------------------------------------------------------
\ I am trying to stay away from using C!, as this way allows for the
\ use of the higher color depths, while using C! does not.
\ -------------------------------------------------------------------

: Draw-Filled-Box { *dst _x1 _y1 _x2 _y2 _pixel -- }
  _y2 1+ _y1 do                 \           from top to bottom of box
    *dst                        \ *dst             surface to plot to
    _x1 _x2                     \ *dst x1 x2       start/end x coords
    i                           \ *dst x1 x2 y                y coord
    _pixel                      \ *dst x1 x2 y pixel      pixel color
    HLine                       \ --
  loop
;
...
: Splash-Screen ( -- )
  640x480x32 s" SDL Library v1.0d" InitGraph  \ create display window
  s" sdllib-10d.png0" Terminate-String sdl-loadimage    \ load image
