...
\
\ 04/21/2010    Added keyboard event functions
\
\ 05/02/2010    Added various interface calls                rev 1.0e
\
...
\ sdl-enable-key-repeat ( delay interval -- f )
\ sdl-getmousestate    ( *x *y -- buttons )
\ sdl-getrelmousestate ( *x *y -- buttons )
...
c-function sdl-enable-key-repeat SDL_EnableKeyRepeat    n n -- n
c-function sdl-getmousestate    SDL_GetMouseState       a a -- n
c-function sdl-getrelmousestate SDL_GetRelativeMouseState a a -- n
