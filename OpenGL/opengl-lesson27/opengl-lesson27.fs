...
\ ---[ =: ]----------------------------------------------------------
\ Redefinition of the CONSTANT function, ala Marcel Hendrix
...
\ Enable the OpenGL Stencil Buffer                             NEW!!!
: Init-Stenciling ( -- )
  SDL_GL_DEPTH_SIZE screen-bpp sdl-gl-set-attribute
  SDL_GL_STENCIL_SIZE 8 sdl-gl-set-attribute
;
...
: Init-Caption ( -- )
  s" gforth/OpenGL: NeHe Lesson 27: Shadows0"
  zstring NULL sdl-wm-set-caption
;
...
: ResizeWindow { _width _height -- boolean }
  ...
  45e0 _width S>F _height S>F F/ 0.001e0 100e0 glu-perspective
...
: Help-Msg ( -- )
  ...
  cr ."     PgUp  increase z-speed"                     1+
  cr ."     PgDn  decrease z-speed"                     1+
  cr ."     ijkl  adjust x,y position of the light"     1+
  cr ."     ou    adjust z position of the light"       1+
  cr ."     wsad  adjust x,y position of the ball"      1+
  cr ."     eq    adjust z position of the ball"        1+
  cr ."   Arrow keys"                                   1+
  cr ."     Up    decrease x-speed"                     1+
  cr ."     Down  increase x-speed"                     1+
  cr ."     Left  decrease y-speed"                     1+
  cr ."     Right increase y-speed"                     1+
  cr ."   Numeric Keypad"                               1+
  cr ."     4658     adjust x,y position of the object" 1+
  cr ."     79       adjust z position of the object"   1+
...
: lesson27 ( -- )
  ...
  Init-Stenciling                          \ Set up OpenGL Stenciling
...
cr ." Made it through the initializations part"
...
cr .( gforth/OpenGL: NeHe Lesson 27)
cr .( type "lesson27" to execute)
cr .( You may need to increase your terminal window size a bit)
cr
