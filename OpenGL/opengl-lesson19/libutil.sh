...
#       files in the PWD.
# --- --- --- --- --- --- --- --- --- --- --- ---  --- --- --- --- --
# Realized a problem after posting this with Lesson 6.
# If the UseLibUtil flag is set to 0, so that the files from the PWD
# are compiled, not the ones in ~/.gforth/opengl-libs, the code in
# this script will not remove those files later if you try, unless
# the -kill option is used.
# I will need to change the file check to allow for the extra two
# digits in the file names if that option has been used.
# --- --- --- --- --- --- --- --- --- --- --- ---  --- --- --- --- --
